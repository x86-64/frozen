...
static ssize_t data_off_t_resize(data_t *data, fastcall_resize *fargs){ // {{{
	return 0;
} // }}}
static ssize_t data_off_t_free(data_t *data, fastcall_free *fargs){ // {{{
	return 0;
} // }}}
static ssize_t data_off_t_view(data_t *data, fastcall_view *fargs){ // {{{
	switch(fargs->format){
		case FORMAT(native):;
			fargs->ptr    = &data->ptr;
...
off_t * data_off_t_alloc(off_t value){ // {{{
	...
	if( (ptr = malloc(sizeof(off_t))) == NULL)
		return NULL;
...
static ssize_t data_off_t_compare(data_t *data1, fastcall_compare *fargs){ // {{{
	...
	data1_val = DEREF_TYPE_OFFT(data1);
	data2_val = DEREF_TYPE_OFFT(fargs->data2); 
...
static ssize_t data_off_t_arith(data_t *data1, fastcall_arith *fargs){ // {{{
	...
	operand1_val = DEREF_TYPE_OFFT(data1);
	operand2_val = DEREF_TYPE_OFFT(fargs->data2); 
	...
	SET_TYPE_OFFT(data1) = (void *)(uintmax_t)result;
	return ret;
} // }}}
...
static ssize_t data_off_t_convert_to(data_t *src, fastcall_convert_to *fargs){ // {{{
	...
	switch( fargs->format ){
		case FORMAT(native):;
		case FORMAT(packed):;
			fastcall_write r_write = { { 5, ACTION_WRITE }, 0, &src_val, sizeof(off_t) };
...
static ssize_t data_off_t_convert_from(data_t *dst, fastcall_convert_from *fargs){ // {{{
	...
	if(dst->ptr == NULL){ // no buffer, alloc new
		if( (dst->ptr = malloc(sizeof(off_t))) == NULL)
			return -ENOMEM;
	}
	#endif
	...
	switch( fargs->format ){
		...
		case FORMAT(human):; // TODO fix it for slider_t 
			...
			SET_TYPE_OFFT(dst) = (void *)((uintmax_t)((off_t )strtoul(buffer, &endptr, 10)));
		...
		case FORMAT(packed):;
			if(fargs->src->type == dst->type){
				SET_TYPE_OFFT(dst) = (void *)((uintmax_t) DEREF_TYPE_OFFT(fargs->src));
				ret = 0;
			}else{
				...
				SET_TYPE_OFFT(dst) = (void *)((uintmax_t)(*((off_t *)buffer)));
			}
			transfered = sizeof(off_t);
			break;
...
static ssize_t data_off_t_is_null(data_t *data, fastcall_is_null *fargs){ // {{{
	fargs->is_null = (data->ptr == NULL || DEREF_TYPE_OFFT(data) == 0) ? 1 : 0;
	return 0;
} // }}}
...
data_proto_t off_t_proto = {
	...
	.handlers               = {
		[ACTION_RESIZE]         = (f_data_func)&data_off_t_resize,
		[ACTION_FREE]           = (f_data_func)&data_off_t_free,
		...
		#ifdef OPTIMIZE_UINT
		[ACTION_VIEW]           = (f_data_func)&data_off_t_view,
		#else
