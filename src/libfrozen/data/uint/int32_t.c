...
static ssize_t data_int32_t_len(data_t *data, fastcall_len *fargs){ // {{{
	fargs->length = sizeof(int32_t);
	return 0;
} // }}}
static ssize_t data_int32_t_alloc(data_t *data, fastcall_alloc *fargs){ // {{{
...
static ssize_t data_int32_t_convert_from(data_t *dst, fastcall_convert_from *fargs){ // {{{
	...
	if(dst->ptr == NULL){ // no buffer, alloc new
		if(data_int32_t_alloc(dst, NULL) != 0)
			return -ENOMEM;
	}
	...
	switch( fargs->src->type ){
		case TYPE_STRINGT:; // TODO fix it for slider_t 
			...
			*(int32_t *)(dst->ptr) = (int32_t )strtoul(buffer, NULL, 10);
			return 0;
		default:;
...
static ssize_t data_int32_t_transfer(data_t *src, fastcall_transfer *fargs){ // {{{
	ssize_t                ret;
...
data_proto_t int32_t_proto = {
	...
	.handlers               = {
		[ACTION_ALLOC]          = (f_data_func)&data_int32_t_alloc,
		[ACTION_PHYSICALLEN]    = (f_data_func)&data_int32_t_len,
		[ACTION_LOGICALLEN]     = (f_data_func)&data_int32_t_len,
		...
		[ACTION_TRANSFER]       = (f_data_func)&data_int32_t_transfer,
