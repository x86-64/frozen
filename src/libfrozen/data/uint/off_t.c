...
static ssize_t data_off_t_len(data_t *data, fastcall_len *fargs){ // {{{
...
static ssize_t data_off_t_transfer(data_t *src, fastcall_transfer *fargs){ // {{{
	ssize_t                ret;
...
data_proto_t off_t_proto = {
	...
	.handlers               = {
		...
		[ACTION_PHYSICALLEN]    = (f_data_func)&data_off_t_len,
		[ACTION_LOGICALLEN]     = (f_data_func)&data_off_t_len,
		...
		[ACTION_TRANSFER]       = (f_data_func)&data_off_t_transfer,
