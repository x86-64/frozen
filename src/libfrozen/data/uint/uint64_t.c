...
static ssize_t data_uint64_t_len(data_t *data, fastcall_len *fargs){ // {{{
	fargs->length = sizeof(uint64_t);
	return 0;
} // }}}
static ssize_t data_uint64_t_alloc(data_t *data, fastcall_alloc *fargs){ // {{{
...
static ssize_t data_uint64_t_compare(data_t *data1, fastcall_compare *fargs){ // {{{
	...
	data1_val = *(uint64_t *)(data1->ptr);
	data2_val = *(uint64_t *)(fargs->data2->ptr); 
...
static ssize_t data_uint64_t_arith(data_t *data1, fastcall_arith *fargs){ // {{{
	...
	operand1_val = *(uint64_t *)(data1->ptr);
	operand2_val = *(uint64_t *)(fargs->data2->ptr); 
...
static ssize_t data_uint64_t_convert_to(data_t *src, fastcall_convert_to *fargs){ // {{{
	...
	switch( src->type ){
		default:;
			fastcall_write r_write = { { 5, ACTION_WRITE }, 0, src->ptr, sizeof(uint64_t) };
...
static ssize_t data_uint64_t_convert_from(data_t *dst, fastcall_convert_from *fargs){ // {{{
	...
	if(dst->ptr == NULL){ // no buffer, alloc new
		if(data_uint64_t_alloc(dst, NULL) != 0)
			return -ENOMEM;
	}
	...
	switch( fargs->src->type ){
		case TYPE_STRINGT:; // TODO fix it for slider_t 
			...
			*(uint64_t *)(dst->ptr) = (uint64_t )strtoul(buffer, NULL, 10);
			return 0;
		default:;
			...
			*(uint64_t *)(dst->ptr) = *((uint64_t *)buffer);
			return 0;
	};
	return -ENOSYS;
} // }}}
static ssize_t data_uint64_t_transfer(data_t *src, fastcall_transfer *fargs){ // {{{
	ssize_t                ret;
...
static ssize_t data_uint64_t_is_null(data_t *data, fastcall_is_null *fargs){ // {{{
	fargs->is_null = (data->ptr == NULL || *(uint64_t *)data->ptr == 0) ? 1 : 0;
	return 0;
} // }}}
...
data_proto_t uint64_t_proto = {
	...
	.handlers               = {
		[ACTION_ALLOC]          = (f_data_func)&data_uint64_t_alloc,
		[ACTION_PHYSICALLEN]    = (f_data_func)&data_uint64_t_len,
		[ACTION_LOGICALLEN]     = (f_data_func)&data_uint64_t_len,
		...
		[ACTION_TRANSFER]       = (f_data_func)&data_uint64_t_transfer,
