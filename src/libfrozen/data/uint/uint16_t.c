...
static ssize_t data_uint16_t_len(data_t *data, fastcall_len *fargs){ // {{{
	fargs->length = sizeof(uint16_t);
	return 0;
} // }}}
static ssize_t data_uint16_t_alloc(data_t *data, fastcall_alloc *fargs){ // {{{
...
static ssize_t data_uint16_t_arith(data_t *data1, fastcall_arith *fargs){ // {{{
	int           ret = 0;
	uint16_t          operand1_val, operand2_val, result;
	...
	switch(fargs->header.action){
		case ACTION_ADD:
			...
			break;
		case ACTION_SUB:
			if(__MIN(uint16_t) + operand2_val > operand1_val)
				ret = -EOVERFLOW;
		...
		case ACTION_MULTIPLY:
			if(operand2_val == 0){
				...
				if(operand1_val > __MAX(uint16_t) / operand2_val)
					ret = -EOVERFLOW;
...
static ssize_t data_uint16_t_convert_from(data_t *dst, fastcall_convert_from *fargs){ // {{{
	...
	if(dst->ptr == NULL){ // no buffer, alloc new
		if(data_uint16_t_alloc(dst, NULL) != 0)
			return -ENOMEM;
	}
...
static ssize_t data_uint16_t_transfer(data_t *src, fastcall_transfer *fargs){ // {{{
	ssize_t                ret;
...
data_proto_t uint16_t_proto = {
	.type                   = TYPE_UINT16T,
	.type_str               = "uint16_t",
	.api_type               = API_HANDLERS,
	.handlers               = {
		[ACTION_ALLOC]          = (f_data_func)&data_uint16_t_alloc,
		[ACTION_PHYSICALLEN]    = (f_data_func)&data_uint16_t_len,
		[ACTION_LOGICALLEN]     = (f_data_func)&data_uint16_t_len,
		[ACTION_COMPARE]        = (f_data_func)&data_uint16_t_compare,
		[ACTION_ADD]            = (f_data_func)&data_uint16_t_arith,
		[ACTION_SUB]            = (f_data_func)&data_uint16_t_arith,
		[ACTION_MULTIPLY]       = (f_data_func)&data_uint16_t_arith,
		[ACTION_DIVIDE]         = (f_data_func)&data_uint16_t_arith,
		[ACTION_INCREMENT]      = (f_data_func)&data_uint16_t_arith_no_arg,
		[ACTION_DECREMENT]      = (f_data_func)&data_uint16_t_arith_no_arg,
		[ACTION_CONVERT_TO]     = (f_data_func)&data_uint16_t_convert_to,
		[ACTION_CONVERT_FROM]   = (f_data_func)&data_uint16_t_convert_from,
		[ACTION_TRANSFER]       = (f_data_func)&data_uint16_t_transfer,
