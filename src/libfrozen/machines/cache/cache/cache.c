...
static ssize_t cache_machine_delete(machine_t *machine, request_t *request){ // {{{
	...
	hash_data_get(ret, TYPE_OFFT,  offset, request, HK(offset));  if(ret != 0) return error("offset not supplied");
	hash_data_get(ret, TYPE_SIZET, size,   request, HK(size));    if(ret != 0) return error("size not supplied");
