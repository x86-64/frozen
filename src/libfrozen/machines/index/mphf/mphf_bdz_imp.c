...
typedef enum bdz_imp_datas {
	BDZ_IMP_DATA_PARAMS = 0,
	BDZ_IMP_DATA_G      = 1
} bdz_imp_datas;
...
static ssize_t  mphf_bdz_imp_getg(bdz_imp_t *data, mphf_t *mphf, char *key, size_t key_len, gval *array){
	...
	tmp = 0;
	mphf_data_read(mphf, BDZ_IMP_DATA_G, array[0].offset, &tmp, data->value_bytes);
	array[0].occupied = ( (tmp & occupied_mask) == 0) ? 0 : 1;
	array[0].value    = tmp & ~occupied_mask;
	
	tmp = 0;
	mphf_data_read(mphf, BDZ_IMP_DATA_G, array[1].offset, &tmp, data->value_bytes);
	array[1].occupied = ( (tmp & occupied_mask) == 0) ? 0 : 1;
	array[1].value    = tmp & ~occupied_mask;
	
	tmp = 0;
	mphf_data_read(mphf, BDZ_IMP_DATA_G, array[2].offset, &tmp, data->value_bytes);
...
ssize_t mphf_bdz_imp_insert (mphf_t *mphf, char *key, size_t key_len, uint64_t  value){
	...
	mphf_data_read   (mphf, BDZ_IMP_DATA_PARAMS, 0, &data, sizeof(bdz_imp_t));
	...
	*g_free = value - (array[0].value + array[1].value + array[2].value);
	...
	if(array[0].occupied == 0){
		...
		mphf_data_write(mphf, BDZ_IMP_DATA_G, array[0].offset, &array[0].value, data.value_bytes);
	}
	if(array[1].occupied == 0){
		array[1].value |= occupied_mask;
		mphf_data_write(mphf, BDZ_IMP_DATA_G, array[1].offset, &array[1].value, data.value_bytes);
	}
	if(array[2].occupied == 0){
		array[2].value |= occupied_mask;
		mphf_data_write(mphf, BDZ_IMP_DATA_G, array[2].offset, &array[2].value, data.value_bytes);
	}
	return 0;
}
...
uint32_t mphf_bdz_imp_ndatas  (config_t *config){
	(void)config;
