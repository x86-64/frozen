...
/**
 * @ingroup machine
 * @addtogroup mod_machine_fixed allocator/fixed
 */
/**
 * @ingroup mod_machine_fixed
 * @page page_fixed_info Description
 *
 * This machine allocate and delete items from storage* machines or from clean datatypes.
 */
/**
 * @ingroup mod_machine_fixed
 ...
 *                   { <memory or file machine to data info>              } 
 *              }
 *
 * }
 * @endcode
...
static int allocator_configure(machine_t *machine, hash_t *config){ // {{{
	...
	if(userdata->item_size == 0){
		...
		hash_data_copy(ret, TYPE_MACHINET,  userdata->removed_items, config, HK(removed_items));
	}
	...
	switch(hargs->action){
		case ACTION_CREATE:;
			...
			if(userdata->removed_items){
				...
				if(machine_fast_query(userdata->removed_items, &r_pop) == 0)
					return 0;
			}
		...
		case ACTION_DELETE:;
			...
			if(userdata->removed_items){
				...
				if(machine_fast_query(userdata->removed_items, &r_push) != 0)
					return -EFAULT;
			}
			return 0;
		...
		case ACTION_COUNT:;
			...
			if(userdata->removed_items){
				...
				if(machine_fast_query(userdata->removed_items, &r_count) == 0){
		...
		case ACTION_WRITE:;
			...
			return machine_fast_pass(machine, r_io);
