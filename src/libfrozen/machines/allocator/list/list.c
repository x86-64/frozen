...
/**
 * @ingroup machine
 * @addtogroup mod_machine_list allocator/list
 */
/**
 * @ingroup mod_machine_list
 * @page page_list_info Description
 *
 * This machine allocate and delete items from storage* machines or from clean datatypes. Data kept in form of list.
 * Items can change their id's after delete actions.
 *
 * This machine support PUSH/POP api set.
 */
/**
 * @ingroup mod_machine_list
...
static ssize_t   allocator_getnewid(machine_t *machine, uintmax_t *newid){ // {{{
	ssize_t                ret;
	allocator_userdata    *userdata          = (allocator_userdata *)machine->userdata;
	...
	*newid = userdata->last_id++;
	...
	if( (ret = machine_fast_pass(machine, &r_resize)) < 0){
		userdata->last_id--;
		return ret;
	}
	return 0;
} // }}}
static ssize_t   allocator_removelastid(machine_t *machine){ // {{{
	ssize_t                ret;
	uintmax_t              id;
	allocator_userdata    *userdata          = (allocator_userdata *)machine->userdata;
...
static int allocator_init(machine_t *machine){ // {{{
	allocator_userdata    *userdata;
	
	if((userdata = machine->userdata = calloc(1, sizeof(allocator_userdata))) == NULL)
		return error("calloc failed");
	
	pthread_rwlock_init(&userdata->rwlock, NULL);
	return 0;
} // }}}
static int allocator_destroy(machine_t *machine){ // {{{
	allocator_userdata    *userdata          = (allocator_userdata *)machine->userdata;
...
static int allocator_configure(machine_t *machine, hash_t *config){ // {{{
	ssize_t                ret;
	data_t                *sample;
	allocator_userdata    *userdata          = (allocator_userdata *)machine->userdata;
	...
	if(userdata->item_size == 0){
		...
		if( machine_fast_pass(machine, &r_len) != 0)
...
static ssize_t allocator_fast_handler(machine_t *machine, fastcall_header *hargs){
	ssize_t                ret;
	uintmax_t              id;
	allocator_userdata    *userdata          = (allocator_userdata *)machine->userdata;
	...
	switch(hargs->action){
		case ACTION_CREATE:;
			...
			pthread_rwlock_wrlock(&userdata->rwlock);
				...
				ret = allocator_getnewid(machine, &r_create->offset);
		...
		case ACTION_WRITE:;
			...
			pthread_rwlock_rdlock(&userdata->rwlock);
				...
				ret = machine_fast_pass(machine, r_io);
		...
		case ACTION_DELETE:;
			...
			pthread_rwlock_wrlock(&userdata->rwlock);
				...
				// TODO move items
				//ret = allocator_removelastid(machine);
		...
		case ACTION_PUSH:;
			...
			pthread_rwlock_wrlock(&userdata->rwlock);
				...
				if( (ret = allocator_getnewid(machine, &id)) < 0)
					break;
				...
				if( (ret = machine_fast_pass(machine, &r_write)) < 0)
					goto push_unwind;
		...
		case ACTION_POP:;
			...
			pthread_rwlock_wrlock(&userdata->rwlock);
				...
				if( (ret = machine_fast_pass(machine, &r_read)) < 0)
					break;
...
/*static ssize_t allocator_handler(machine_t *machine, request_t *request){
	ssize_t                ret               = 0;
	uintmax_t              action;
