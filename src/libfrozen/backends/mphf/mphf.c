...
mphf_types      mphf_type_from_string(char *string){
...
static int mphf_configure(chain_t *chain, hash_t *config){ // {{{
	...
	hash_copy_data(config, TYPE_INT64,  "n_initial",  n_initial,     N_INITIAL_DEFAULT);
	hash_copy_data(config, TYPE_INT32,  "value_bits", value_bits,    VALUE_BITS_DEFAULT);
	hash_copy_data(config, TYPE_STRING, "mphf_type",  mphf_type_str, NULL);
	hash_copy_data(config, TYPE_STRING, "key",        key_str,       "key");
	...
	// create new
	if( (mphf_type = mphf_type_from_string(mphf_type_str)) == MPHF_TYPE_INVALID)
...
static ssize_t mphf_backend_insert(chain_t *chain, request_t *request){
	...
	if( (key = hash_get_data(request, userdata->key)) == NULL)
		return chain_next_query(chain, request);
	
	request_t r_next[] = {
		{ userdata->key,  DATA_VOID                },
		{ "key_out",      DATA_PTR_INT64(&key_out) },
		hash_next(request)
	};
...
static ssize_t mphf_backend_query(chain_t *chain, request_t *request){
	...
	switch(mphf_query(&userdata->main_mphf, data_value_ptr(key), data_value_len(key), &key_out)){
		...
		case MPHF_QUERY_FOUND:;
			request_t r_next[] = {
				{ userdata->key,  DATA_PTR_INT64(&key_out) },
				hash_next(request)
			};
