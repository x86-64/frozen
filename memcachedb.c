...
static void item_output(conn *c, char *item_name, char *item_data, size_t item_data_len){
	char key_header[255];
	sprintf((char *)&key_header, "VALUE %s 0 %u\r\n", item_name, (unsigned int)item_data_len);

	if(
		add_iov(c, key_header, strlen(key_header)) != 0 ||
		add_iov(c, item_data, item_data_len)       != 0 ||
		add_iov(c, "\r\n", 2)                      != 0
	){
		printf("ERROR: can't send item\n");
		out_string(c, "SERVER_ERROR can't send item");
	}
}
...
static void process_set_command(conn *c, ztoken_t *tokens, size_t ntokens){
	...
	item_new->data_len = strtol(tokens[4].value, NULL, 10);
	item_new->data     = malloc(item_new->data_len + 2);
	if(item_new->data == NULL){
		out_string(c, "SERVER_ERROR insufficient memory");
		dbitem_free(item_new);
		return;
	}
...
static void process_command(conn *c, char *command) {
    ...
    if (ntokens == 0 || tokens[COMMAND_TOKEN].value == NULL){
    	out_string(c, "ERROR");
	return;
    }
...
static void drive_machine(conn *c) {
	...
	case conn_nread:
            ...
            if (c->rlbytes == 0) {
                process_set_command_complete(c);
		break;
            }
...
int main (int argc, char **argv) {
    ...
    /* process arguments */
    while ((c = getopt(argc, argv, "a:U:p:s:c:hvl:dru:P:t:b:D:")) != -1) {
        switch (c) {
        case 'a':
